...
$(document).ready(function () {
    ...
    try {
        $('.table_Common').each(function (i, e) { fnLoadCommonTable(e); })
    } catch { }
    ...
    if (formValidate('#' + $id)) {
        ...
        if (typeof inputFiles !== 'undefined' && inputFiles !== null && inputFiles.length > 0) {
            ...
            $.each(inputFiles, function (key, input) {
                ...
                if ($(input).hasClass('temp') || $(input).hasClass('temp_fileUpload'))
                    return;
...
function fnLoadCommonTable($selector) {
    ...
    $($selector).find("thead th.no_sorting").removeClass('sorting');
    $($selector).find("thead th.no_sorting").removeClass('sorting_asc');
    $($selector).find("thead th.no_sorting").removeClass('sorting_desc');
}
