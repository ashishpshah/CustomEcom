...
{
	[Area("Admin")]
	public class AttributeController : BaseController<ResponseModel<Attribute>>
	{
					...
					if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
						foreach (DataRow dr in ds.Tables[1].Rows)
							result.Add(new Attribute()
							{
								...
								Values = GetValue<string>(dr, "AttributeValues"),
								Values_Ids = GetValue<string>(dr, "AttributeValues_Ids"),
		...
		[HttpPost]
		public JsonResult Save(Attribute viewModel)
		{
			try
			{
				...
				if (string.IsNullOrEmpty(viewModel.Values))
				{
					CommonViewModel.IsSuccess = false;
				...
				var values = viewModel.Values.Trim();
				...
				oParams.Add(new SqlParameter("DisplayName", viewModel.DisplayName));
				oParams.Add(new SqlParameter("AttributeValues", viewModel.Values));
